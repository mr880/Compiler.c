...
static int variable()
{
	...
	reg = next_register();
	CodeGen(LOADAI, EMPTY_FIELD, (token-'a')*4, reg);
	next_token();
	return reg;
}
...
static void assign()
{
    ...
    if(is_identifier(token)){
        ...
        next_token();
        reg = expr();
        CodeGen(STOREAI, 4*(token - 'a'), reg, EMPTY_FIELD);
    }
    else{
...
static void print()
{
	...
	if(is_identifier(token)){
		...
		CodeGen(OUTPUTAI, 4*(token - 'a'), EMPTY_FIELD, EMPTY_FIELD);
		next_token();
	}else{
...
static void program()
{
	/*checking program for non-terminal symbol "stmtlist"
	and gives an error otherwise and exits the program */
	stmtlist();
    if (token != '.') {
        ERROR("Program error.  Current input symbol is %c\n", token);
        exit(EXIT_FAILURE);
